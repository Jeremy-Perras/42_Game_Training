...
namespace ve {
  ...
  Pipeline::Pipeline(Device& device, VkExtent2D swapChainExtent, VkRenderPass renderPass)
      : _device(device), _swapChainExtent(swapChainExtent), _renderPass(renderPass) {
  ...
  void Pipeline::createGraphicsPipeline() {
    ...
    if (vkCreatePipelineLayout(_device.getDevice(), &pipelineLayoutInfo, nullptr, &_pipelineLayout)
        != VK_SUCCESS) {
