...
namespace ve {
  ...
  TextOverlay::TextOverlay(Device &device, Renderer &renderer, const TextOverlay::Builder &builder)
  ...
  void TextOverlay::createVertexBuffer(const std::vector<Vertex> &vertices) {
  ...
  void TextOverlay::createIndexBuffers(const std::vector<uint32_t> &indices) {
  ...
  void TextOverlay::bind(VkCommandBuffer commandBuffer) {
  ...
  void TextOverlay::draw(VkCommandBuffer commandBuffer) const {
    if (hasIndexBuffer_) {
  ...
  std::vector<VkVertexInputBindingDescription> TextOverlay::Vertex::getBindingDescriptions() {
  ...
  std::vector<VkVertexInputAttributeDescription> TextOverlay::Vertex::getAttributeDescriptions() {
  ...
  void TextOverlay::createPipelineLayout() {
  ...
  void TextOverlay::createPipeline() {
    ...
    pipelineConfig.attributeDescriptions = TextOverlay::Vertex::getAttributeDescriptions();
    pipelineConfig.bindingDescriptions = TextOverlay::Vertex::getBindingDescriptions();

    pipeline_ = std::make_unique<Pipeline>(device_, "shaders/starNest.vert.spv",
                                           "shaders/starNest.frag.spv", pipelineConfig);
  }

  void TextOverlay::renderGameObjects(FrameInfo &frameInfo) {
    ...
    TextOverlay::bind(frameInfo.commandBuffer);
    TextOverlay::draw(frameInfo.commandBuffer);
  }
