...
// 	for (int i = 0; i < 6; ++i) {
// 		float mag = dot(p, p);
// 		p = abs(p) / mag + vec3(-.5, -.8 + 0.1 * sin(pushConstants.iTime * 0.2 + 2.0), -1.1 + 0.3 * cos(pushConstants.iTime * 0.15));
...
void main( )
{
	vec2 p = (2.0*gl_FragCoord.xy-pushConstants.iResolution.xy)/pushConstants.iResolution.y;

    float time = 30.0 + 0.1*pushConstants.iTime;
    ...
    p = gl_FragCoord.xy/pushConstants.iResolution.xy;
